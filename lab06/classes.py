...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        idx = 0
        while idx < len(opponent.deck.cards):
            if opponent.deck.cards[idx].attack == other_card.attack and opponent.deck.cards[idx].defense == other_card.defense:
                del opponent.deck.cards[idx]
            else:
                idx += 1
