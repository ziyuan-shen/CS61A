...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t)==True:
        if label(t)==old:
            ...
            return tree(label(t))
    return tree(label(t), branches = [replace_leaf(branch, old, new) for branch in branches(t)])
...
def balanced(m):
    ...
    left_side = left(m)
    right_side = right(m)
    left_size = length(left_side)
    right_size = length(right_side)
    left_weight = total_weight(end(left_side))
    right_weight = total_weight(end(right_side))
    if (left_size * left_weight) != (right_size * right_weight):
        return False
...
def make_withdraw(balance, password):
    ...
    """
    attempts = []
    def withdraw(amount, password_entered):
        nonlocal balance
        nonlocal attempts
        is_locked = len(attempts)==3
        if is_locked:
            ...
            attempts.append(password_entered)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert (lower_bound(y) * upper_bound(y)) > 0, 'divisor must not span 0'
