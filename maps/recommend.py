...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    latitude = [restaurant_location(r)[0] for r in cluster]
    longitude = [restaurant_location(r)[1] for r in cluster]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    x_mu = mean(xs)
    S_xx = sum([(x - x_mu) ** 2 for x in xs])
    y_mu = mean(ys)
    S_yy = sum([(y - y_mu) ** 2 for y in ys])
    S_xy = sum([(xs[i] - x_mu) * (ys[i] - y_mu) for i in range(len(xs))])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    feature_fn = max(feature_fns, key = lambda fn: find_predictor(user, reviewed, fn)[1])
